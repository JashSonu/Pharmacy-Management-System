...
db=mysql.connector.connect(host="localhost", user="root", passwd="jashpatel1",database="pharmacy")
...
def cartfunc():
    ...
    def addnow():
        sql4 = "INSERT INTO cart(Medicine_id, M_name, Price) SELECT Medicine_id, M_name, Price FROM medicine WHERE M_name= '%s' ;" % (
            'Cold Nova')
        c.execute(sql4)
        db.commit()

        sql3 = "UPDATE cart SET Customer_id = '%d' WHERE (M_name = 'Cold Nova');" % (custid)
        c.execute(sql3)
        db.commit()
    ...
    def proceed():
        ...
        Label(page1, image=ef).place(relwidth=1, relheight=1)
        ex = Button(page1, text="  Exit  ", bd=5, command=page1.destroy).place(x=150, y=600)

    Radiobutton(cartpage, text="Cash on Delivery", font=('Calibri', 16), variable=v, value=1).place(x=10, y=60)
    Radiobutton(cartpage, text="Credit/Debit Card, Net Banking,UPI,Wallet", font=('Calibri', 16), variable=v,
                value=2).place(x=10, y=90)
    Radiobutton(cartpage, text="Credit/Debit Card", font=('Calibri', 16), variable=v, value=3).place(x=10, y=120)
    Radiobutton(cartpage, text="Net Banking", font=('Calibri', 16), variable=v, value=4).place(x=10, y=150)
    Radiobutton(cartpage, text="PayU(Wallet,UPI,GooglePay)", font=('Calibri', 16), variable=v, value=5).place(x=10,
                                                                                                              y=180)

    B = Button(cartpage, text="  Proceed to PAY  ", bd=5, command=addnow).place(x=150, y=600)
        ...
        if(counter<=pages):
            ...
            for i in range(0,4):
                ...
                if(i==0):
                    ...
                    w=Button(f1,text="+",width=2,relief=RIDGE,bg="#CCD6FF",command=prescheck)
                    w.place(x=300,y=84)
                ...
                if(i==1):
                    ...
                    w = Button(f2, text="+", width=2, relief=RIDGE, bg="#EACCFF",command=prescheck1)
                    w.place(x=300, y=84)
                ...
                if(i==2):
                    ...
                    w = Button(f3, text="+", width=2, relief=RIDGE, bg="#FFCCCC",command=prescheck2)
                    w.place(x=300, y=84)
                ...
                if(i==3):
                    ...
                    w = Button(f4, text="+", width=2, relief=RIDGE, bg="#FDDDB3",command=prescheck3)
                    w.place(x=300, y=84)
